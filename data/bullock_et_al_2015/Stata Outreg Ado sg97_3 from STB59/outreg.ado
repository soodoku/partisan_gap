*! version 3.0.6  27nov00  by john_gallup@alum.swarthmore.edu  (STB-59: sg97.3)
...
*                SE Pvalue CI BEtaco Level(integer $S_level) TDec(numlist int >=0 <=11 max=1) 
*                noPAren BRacket noASter 3aster 10pct SIgsymb(passthru)
* 
*                /* statistics options */
...
program define outreg
   ...
   * write formatted regression output to file
   ...
   syntax [varlist(default=none ts)] using [, noLabel TItle(passthru) CTitle(passthru) noNOTes ADDNote(passthru) BDec(numlist int >=0 <=11) BFmt(passthru) COEfastr SE Pvalue CI BEtaco Level(integer $S_level) Tdec(numlist int >=0 <=11 max=1) noPAren BRacket noASter 3aster 10pct SIgsymb(passthru) noCONs noNOBs noNI noR2 ADJr2 RDec(numlist int >=0 <=11 max=1) ADDStat(passthru) ADEc(numlist int >=0 <=11) EForm Margin1 Margin2(string) Onecol Xstats COMma APpend REPLACE Quote] 
   ...
   if `"`addstat'"'!="" {
      ...
      while `"`rest'"' != "" {
         ...
         else {gettoken comma2 rest: rest, parse(",")}
         local value = `valstr'
   ...
   /* bivariate probit, heckman, heckprob */
   else if ("`cmd'"=="biprobit" | "`cmd'"=="heckman" | "`cmd'"=="heckprob") {
      local univar  = 0      /* multivariate (multiple equation) regression */
      if ("`onecol'"!="onecol") {
         local neq     = e(k_dv)
         local eqlist  = e(depvar)
         if "`cmd'"=="heckman" {
            local eqlist "`eqlist' select"
            local depvar "`depvar' select"
         }
      }
      else { /* if onecol */
   ...
   /* multinomial regression */
      ...
      if "`onecol'"!="onecol" {
         ...
         mat `vc' = e(V)
         local k = colsof(`vc')/`neq'
         local catlist : coleq(`vc')  /* list of categories */
         local depvar ""
         local cat = 1
         while `cat' <= `neq' {
   ...
   quietly {
      ...
      if `univar' | "`onecol'"=="onecol" {  /* make univariate regression table */
         ...
         while `eq' <= `neq' {
            ...
            coeftxt `varlist', `se' `pvalue' `ci' `betaco' level(`level') `label' bdec(`bdec') `bfmt' tdec(`tdec') `paren' `bracket' `aster' `3aster' `10pct' `sigsymb' `coefastr' `cons' `eform' `nobs' `ni' `r2' `adjr2' rdec(`rdec') `title' `ctitle' addstat(`addstat') adec(`adec') `notes' `addnote' `apptmp' regN(`regN') df_r(`df_r') rsq(`rsq') numi(`numi') ivar(`ivar') depvar(`depvar') robust(`robust') borows(`bocols') b(`b_eq') vc(`vc_eq') varname(`varname') coefcol(`coefcol') univar(`univar')
...
program define coeftxt
   ...
   * convert vectors b and vc into coefficient and t statistics text + variable name text
   ...
   if `"`ctitle'"' != "" {local coltitl `"`ctitle'"'}
      ...
      if "`label'"=="" {
         capture local coltitl : var label `depvar'
         }
   ...
   while "`bname'"!="" {
      if "`bname'"!="_cons" {
         if "`label'"!="nolabel" {
            ...
            else {local blabel : var label `bname'}
         }
         else {local blabel ""}
   ...
   local totrows = `coefrow' + ("`nobs'"!="nonobs") + (`numi'!=.) + /*
